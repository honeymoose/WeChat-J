...
public class WxMpStoreServiceImpl implements WxMpStoreService {
  ...
  public void add(WxMpStoreBaseInfo request) throws WxErrorException {
    ...
    String response = this.weChatOfficialAccountService.post(WxMpApiUrl.Store.POI_ADD_URL, request.toJson());
  ...
  public WxMpStoreBaseInfo get(String poiId) throws WxErrorException {
    ...
    String response = this.weChatOfficialAccountService.post(WxMpApiUrl.Store.POI_GET_URL, paramObject.toString());
    WxError wxError = WxError.fromJson(response, WxType.MP);
    if (wxError.getErrorCode() != 0) {
      throw new WxErrorException(wxError);
    }
  ...
  public void delete(String poiId) throws WxErrorException {
    JsonObject paramObject = new JsonObject();
    paramObject.addProperty("poi_id", poiId);
    String response = this.weChatOfficialAccountService.post(WxMpApiUrl.Store.POI_DEL_URL, paramObject.toString());
    WxError wxError = WxError.fromJson(response, WxType.MP);
    if (wxError.getErrorCode() != 0) {
      throw new WxErrorException(wxError);
    }
  ...
  public WxMpStoreListResult list(int begin, int limit)
    ...
    String response = this.weChatOfficialAccountService.post(WxMpApiUrl.Store.POI_LIST_URL, params.toString());
  ...
  public void update(WxMpStoreBaseInfo request) throws WxErrorException {
    String response = this.weChatOfficialAccountService.post(WxMpApiUrl.Store.POI_UPDATE_URL, request.toJson());
    WxError wxError = WxError.fromJson(response, WxType.MP);
    if (wxError.getErrorCode() != 0) {
      throw new WxErrorException(wxError);
    }
  ...
  public List<String> listCategories() throws WxErrorException {
    String response = this.weChatOfficialAccountService.get(WxMpApiUrl.Store.POI_GET_WX_CATEGORY_URL, null);
    WxError wxError = WxError.fromJson(response, WxType.MP);
    if (wxError.getErrorCode() != 0) {
      throw new WxErrorException(wxError);
    }
