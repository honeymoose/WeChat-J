...
import com.ossez.wechat.wecom.api.*;
import com.ossez.wechat.wecom.bean.WxCpAgentJsapiSignature;
import com.ossez.wechat.wecom.bean.WxCpMaJsCode2SessionResult;
import com.ossez.wechat.wecom.bean.WxCpProviderToken;
...
public abstract class BaseWxCpServiceImpl<H, P> implements WxCpService, RequestHttp<H, P> {
  ...
  public String getAgentJsapiTicket(boolean forceRefresh) throws WxErrorException {
    ...
    if (this.configStorage.isAgentJsapiTicketExpired()) {
      synchronized (this.globalAgentJsapiTicketRefreshLock) {
        if (this.configStorage.isAgentJsapiTicketExpired()) {
          String responseContent = this.get(this.configStorage.getApiUrl(WxCpApiPathConsts.GET_AGENT_CONFIG_TICKET), null);
  ...
  public String getJsapiTicket(boolean forceRefresh) throws WxErrorException {
    ...
    if (this.configStorage.isJsapiTicketExpired()) {
      synchronized (this.globalJsapiTicketRefreshLock) {
        if (this.configStorage.isJsapiTicketExpired()) {
          String responseContent = this.get(this.configStorage.getApiUrl(WxCpApiPathConsts.GET_JSAPI_TICKET), null);
  ...
  public WxCpMaJsCode2SessionResult jsCode2Session(String jsCode) throws WxErrorException {
    ...
    final String url = this.configStorage.getApiUrl(WxCpApiPathConsts.JSCODE_TO_SESSION);
    return WxCpMaJsCode2SessionResult.fromJson(this.get(url, Joiner.on("&").withKeyValueSeparator("=").join(params)));
  }

  @Override
  public String[] getCallbackIp() throws WxErrorException {
    String responseContent = get(this.configStorage.getApiUrl(WxCpApiPathConsts.GET_CALLBACK_IP), null);
    JsonObject tmpJsonObject = GsonParser.parse(responseContent);
  ...
  public WxCpProviderToken getProviderToken(String corpId, String providerSecret) throws WxErrorException {
    ...
    return WxCpProviderToken.fromJson(this.post(this.configStorage.getApiUrl(WxCpApiPathConsts.Tp.GET_PROVIDER_TOKEN),
      jsonObject.toString()));
  }
  ...
  public String replaceParty(String mediaId) throws WxErrorException {
    ...
    return post(this.configStorage.getApiUrl(WxCpApiPathConsts.BATCH_REPLACE_PARTY), jsonObject.toString());
  }
  ...
  public String syncUser(String mediaId) throws WxErrorException {
    JsonObject jsonObject = new JsonObject();
    jsonObject.addProperty("media_id", mediaId);
    String responseContent = post(this.configStorage.getApiUrl(WxCpApiPathConsts.BATCH_SYNC_USER), jsonObject.toString());
  ...
  public String replaceUser(String mediaId) throws WxErrorException {
    JsonObject jsonObject = new JsonObject();
    jsonObject.addProperty("media_id", mediaId);
    return post(this.configStorage.getApiUrl(WxCpApiPathConsts.BATCH_REPLACE_USER), jsonObject.toString());
  }

  @Override
  public String getTaskResult(String jobId) throws WxErrorException {
    String url = this.configStorage.getApiUrl(WxCpApiPathConsts.BATCH_GET_RESULT + jobId);
    return get(url, null);
  }
